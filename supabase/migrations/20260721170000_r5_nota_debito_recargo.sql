-- ============================================================
-- R5 — Nota de Débito: recargo/interés como línea de concepto libre.
--
-- La ND se usa para cobrar de más (interés/mora). Decisión de negocio: la ND NO
-- trae productos, es UN cargo sobre la factura. El front calcula el recargo
-- (% sobre el total con IVA de la factura + monto fijo) y lo manda como una línea
-- SIN producto, con IVA 21%.
--
-- Para eso venta_items.producto_id pasa a NULLABLE y crear_venta acepta líneas de
-- concepto libre SÓLO en NOTA_DEBITO (guarda explícita): así ningún producto_id
-- NULL llega jamás a stock_sucursal/stock_movimientos (NOT NULL) — la ND no mueve
-- stock ni se puede anular (anular_venta sólo anula FACTURA_*). codigo sigue NOT
-- NULL, la línea usa 'RECARGO'.
--
-- crear_venta: CREATE OR REPLACE partiendo de la versión de R6 (20260721140000),
-- que ya trae R3 (centavo), R2.a (factura interna) y R6 (venta sin stock).
-- ============================================================

-- 1) producto_id nullable para permitir líneas de concepto libre (recargo).
ALTER TABLE public.venta_items ALTER COLUMN producto_id DROP NOT NULL;

-- 2) crear_venta con la rama de concepto libre.
