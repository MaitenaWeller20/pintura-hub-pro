-- ============================================================
-- Remito de obra sin tener que elegir un cliente.
--
-- May: "las obras no las guardamos como clientes, yo pongo acá el nombre...
-- pero no me deja grabar hasta que no elija un cliente".
--
-- No alcanza con sacar la validación. REMITO_OBRA va a CUENTA CORRIENTE
-- (`v_es_cta_cte := p_tipo_comprobante IN ('REMITO','REMITO_OBRA') ...`) y la
-- cuenta corriente cuelga del cliente: sin cliente, la deuda no tiene dueño, no
-- se le puede cobrar a nadie y no aparece en ningún saldo. En la foto que mandó
-- eran $183.510.
--
-- Solución: la obra ES el cliente, creado solo. Al guardar un remito de obra sin
-- cliente elegido, se busca (o se crea) una ficha con el nombre de la obra y la
-- venta se le asigna. Así cada obra tiene su cuenta corriente y su saldo
-- cobrable, y May sólo escribe el nombre.
--
-- DECISIONES QUE IMPORTAN, y por qué:
--
--   * Las fichas de obra se marcan con `es_obra`, NO con `es_generico`. Ese flag
--     ya significa otra cosa (los consumidores finales sembrados) y la UI lo
--     muestra como "Genérico".
--
--   * NO se reusa un cliente real que se llame igual. Si "Distrito SRL" existe
--     como cliente y alguien carga una obra "Distrito SRL", asignarle la deuda
--     de la obra a esa empresa es un error contable, no una molestia. El índice
--     único y la búsqueda son sólo entre fichas de obra.
--
--   * La ficha de obra NO lleva `condicion_cta_cte = true`. Dos razones: el
--     trigger `guard_clientes_credito` le prohíbe a un empleado crear clientes
--     con cuenta corriente —y `SECURITY DEFINER` no lo esquiva, porque el guard
--     decide con `auth.uid()`, que dentro de la RPC sigue siendo el empleado—; y
--     no hace falta, porque la vista `cuenta_corriente_saldos` incluye a
--     cualquiera con saldo distinto de cero. La deuda de la obra se ve y se
--     cobra igual, sin abrirle crédito a nadie por la API.
--
--   * La unicidad la garantiza un índice, no un SELECT previo: dos remitos a la
--     misma obra al mismo tiempo crearían dos fichas. Con el índice + ON
--     CONFLICT, el segundo reusa la del primero.
--
-- Ver docs/superpowers/specs/2026-08-12-correcciones-may-design.md
-- ============================================================

-- ------------------------------------------------------------
-- 1. Marca de ficha de obra
-- ------------------------------------------------------------
ALTER TABLE public.clientes
  ADD COLUMN IF NOT EXISTS es_obra boolean NOT NULL DEFAULT false;

COMMENT ON COLUMN public.clientes.es_obra IS
  'La ficha representa una OBRA, no un cliente real. La crea sola crear_venta al '
  'guardar un remito de obra, para que la deuda de la obra tenga dueño y se pueda '
  'cobrar. No confundir con es_generico (consumidor final sin identificar).';

-- ------------------------------------------------------------
-- 2. Nombre comparable
-- ------------------------------------------------------------
-- Espejo de `claveNombre` en src/lib/importar-clientes.ts: sin acentos, sin
-- mayúsculas, sin espacios de más. Así "Obra Distrito" y "OBRA  DISTRITO" son la
-- misma obra y no se duplican.
--
-- `translate` y no `unaccent`: la extensión no está instalada en producción y no
-- vale la pena agregarla para esto. IMMUTABLE para poder indexarla.
CREATE OR REPLACE FUNCTION public.clave_nombre(v text)
RETURNS text
LANGUAGE sql
IMMUTABLE
AS $$
...
-- Una obra activa por nombre. Parcial: no toca a los clientes de verdad.
CREATE UNIQUE INDEX IF NOT EXISTS uq_clientes_obra_nombre
  ON public.clientes ((public.clave_nombre(razon_social)))
  WHERE es_obra AND activo;

-- ------------------------------------------------------------
-- 3. Resolver la obra dentro de crear_venta
-- ------------------------------------------------------------
-- Va acá y no en el navegador porque tiene que pasar en la MISMA transacción que
-- la venta: si la ficha se creara aparte y la venta fallara, quedaría una obra
-- fantasma sin movimientos.
CREATE OR REPLACE FUNCTION public.resolver_cliente_obra(p_nombre text)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
AS $$
DECLARE
  v_clave text := public.clave_nombre(p_nombre);
  v_id    uuid;
BEGIN
  IF v_clave = '' THEN
    RAISE EXCEPTION 'Poné el nombre de la obra.';
  END IF;

  SELECT id INTO v_id
    FROM public.clientes
   WHERE es_obra AND activo AND public.clave_nombre(razon_social) = v_clave;
  IF v_id IS NOT NULL THEN
    RETURN v_id;
  END IF;

  -- ON CONFLICT contra el índice parcial: si otro remito a la misma obra la creó
  -- entre el SELECT y el INSERT, no explota — se relee abajo.
  INSERT INTO public.clientes (razon_social, tipo, es_obra, condicion_cta_cte)
  VALUES (btrim(p_nombre), 'CONSUMIDOR_FINAL', true, false)
...
REVOKE ALL ON FUNCTION public.resolver_cliente_obra(text) FROM public;
GRANT EXECUTE ON FUNCTION public.resolver_cliente_obra(text) TO authenticated;

COMMENT ON FUNCTION public.resolver_cliente_obra(text) IS
  'Devuelve el id de la ficha de la obra con ese nombre, creándola si no existe. '
  'Sólo mira fichas con es_obra: nunca reusa un cliente real homónimo.';

-- ------------------------------------------------------------
-- 4. crear_venta: acepta cliente nulo en un remito de obra
-- ------------------------------------------------------------
-- Cambios respecto de la versión anterior:
--   * si no viene cliente y el comprobante es REMITO_OBRA, se resuelve la ficha
--     de la obra a partir de `p_nombre_obra`;
--   * la exigencia de "cuenta corriente habilitada" no le aplica a una ficha de
--     obra (ver el porqué arriba);
--   * el resto del cuerpo usa `v_cliente_id`, que es el cliente ya resuelto.
-- `p_cliente_id` sigue siendo un parámetro obligatorio de la firma (no puede
-- llevar DEFAULT porque lo siguen parámetros sin default), pero ahora admite NULL.
