-- ============================================================
-- R6 — Venta de productos sin stock, habilitada por PERFIL.
--
-- Antes sólo existía un flag GLOBAL settings.permitir_stock_negativo (todo o
-- nada, sin UI). El negocio necesita que "algunos perfiles puedan registrar
-- ventas con productos sin stock y otros no". Se agrega un flag por usuario en
-- profiles; los admins siempre pueden. La barrera autoritativa sigue en la RPC
-- crear_venta (el front sólo muestra un warning).
--
-- crear_venta: CREATE OR REPLACE partiendo de la versión de R2 (20260721120000),
-- que ya trae R3 (centavo) y R2.a (factura interna de contado).
-- ============================================================

-- 1) Columna de permiso por perfil.
ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS permite_venta_sin_stock boolean NOT NULL DEFAULT false;

COMMENT ON COLUMN public.profiles.permite_venta_sin_stock IS
  'Si true, este usuario puede registrar ventas de productos sin stock disponible (stock negativo). Los admin siempre pueden. Sólo un admin puede cambiarlo.';

-- 2) Un empleado NO puede auto-otorgarse el permiso: lo agregamos al guard de
--    columnas administrativas de profiles.
...
-- 3) Helper: ¿este usuario puede vender sin stock? (admin o flag por perfil).
...
AS $$
  SELECT public.is_admin(_uid)
      OR COALESCE((SELECT permite_venta_sin_stock FROM public.profiles WHERE id = _uid), false);
$$;

REVOKE ALL ON FUNCTION public.puede_vender_sin_stock(uuid) FROM public;
GRANT EXECUTE ON FUNCTION public.puede_vender_sin_stock(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.puede_vender_sin_stock(uuid) TO service_role;

-- 4) crear_venta con el gate por perfil.
