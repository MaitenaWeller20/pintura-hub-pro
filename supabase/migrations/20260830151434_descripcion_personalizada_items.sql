-- Descripción congelada por renglón. El helper se mantiene owner-only:
-- sólo los escritores definer pueden invocarlo.
...
REVOKE ALL ON FUNCTION public._normalizar_descripcion_item_20260830(
  text,text,boolean
) FROM PUBLIC,anon,authenticated,service_role;

-- Última definición efectiva de crear_presupuesto; sólo resuelve y persiste
...
-- Última definición efectiva de editar_presupuesto; una línea existente usa
-- su snapshot como fallback y una línea nueva usa el catálogo.
...
-- Core comercial efectivo, renombrado en 20260823162000. La envoltura pública
-- conserva intacta la autorización y la huella idempotente del payload.
...
BEGIN
  ...
  -- T13 review: el cerco de notas vive antes del replay idempotente, la
  -- numeración, los locks de productos y cualquier efecto comercial. En v2 la
  -- NC fiscal sólo puede nacer por anular_venta; crear_venta conserva las notas
  -- legacy únicamente mientras ese writer sea el único habilitado.
