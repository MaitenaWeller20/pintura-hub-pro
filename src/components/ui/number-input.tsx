    ...
    return (
      <Input
        ...
        onChange={(e) => {
          ...
          if (raw === "" || re.test(raw)) {
            setText(raw);
            // "vacío", "-", "," o "." sin ningún dígito no es un número: hay que
            // emitir null y NO dejar colgado el valor numérico anterior (si no,
            // el consumidor seguiría creyendo que hay un número escrito).
            if (!/\d/.test(raw)) {
              onValueChange(null);
            } else {
