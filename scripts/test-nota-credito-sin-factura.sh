...
if [[ ! "$SETTINGS_ANTES" =~ ^[01]\|[01]$ ]]; then
  echo "No se pudo capturar el estado previo de los writers fiscales." >&2
  exit 1
fi
...
COMMIT;
SQL
  q "UPDATE public.settings SET facturacion_receptor_v2_enabled=('$V2_ANTES'='1'), facturacion_legacy_writer_enabled=('$LEGACY_ANTES'='1') WHERE id=true" >/dev/null 2>&1 || true
}
...
chequear "queda fuera de ARCA y sin CAE" "true" \
  "$(q "select (afip_estado='NO_APLICA' and cae is null and afip_numero is null)::text from public.ventas where id='$NC_CC'")"
chequear "no entra en la cola fiscal" "0" \
  "$(q "select count(*)::text from public.emision_fiscal_intentos where venta_id='$NC_CC'")"
...
chequear "también queda interna y sin asociación" "true" \
  "$(q "select (afip_cbte_asoc_id is null and afip_estado='NO_APLICA' and cae is null)::text from public.ventas where id='$NC_CO'")"
...
rechaza "la rechaza fuera del alcance de v2" "nota de débito nueva queda fuera de alcance fiscal" "$OUT"

echo
echo "── 5. Anular una NC interna revierte stock y cuenta corriente ────────"
S2=$(stock)
OUT=$( { $PSQL <<SQL
$(auth "$MAIL")
...
echo
echo "── 6. Anular una NC pagada NO toca el arqueo de su caja ──────────────"
...
echo
echo "── 7. La NC asociada no se puede crear manualmente en v2 ─────────────"
OUT=$( { $PSQL <<SQL
$(auth "$MAIL")
SELECT * FROM public.crear_venta(
  '$SUC','$CLI','NOTA_CREDITO','CTA_CTE',$ITEM,'[]'::jsonb,0,
  'TEST-NCSF-NCFISCAL-NO-DEBE-PERSISTIR',NULL,NULL,
  '00000000-0000-0000-0000-000000000001');
SQL
} 2>&1 )
rechaza "obliga a usar la anulación fiscal" "nota de crédito v2 se crea exclusivamente mediante anular_venta" "$OUT"
chequear "no deja efectos parciales" "0" \
  "$(q "select count(*)::text from public.ventas where observaciones='TEST-NCSF-NCFISCAL-NO-DEBE-PERSISTIR'")"

echo
echo "── 8. La NC que genera una ANULACIÓN no se puede anular ──────────────"
