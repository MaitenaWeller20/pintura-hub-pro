-- ============================================================
-- crear_venta: lockear los productos en orden antes del loop.
--
-- Hallazgo del review de Codex sobre presupuestos, pero el problema es de
-- crear_venta y es viejo: el loop lockea los productos en el orden en que
-- vienen en p_items. Dos cajas vendiendo los mismos dos productos en distinto
-- orden (una A,B y la otra B,A) se traban cruzadas y Postgres mata una con
-- "deadlock detected". La conversión de presupuestos ordena por producto_id,
-- así que el cruce con una venta manual era todavía más probable.
--
-- Base: la definición VIVA, verificada con diff contra pg_proc — que NO es la
-- de 20260721140000_r6, sino la de 20260721170000_r5_nota_debito_recargo.sql
-- (r5 se aplicó después que r6 y le agregó las líneas de concepto libre).
-- El único cambio es el PERFORM de abajo.
-- ============================================================
