-- ============================================================
-- crear_venta: al contado se cobra entero.
--
-- Una venta CONTADO sin pagos quedaba en PENDIENTE con total_pagado = 0 y el
-- stock ya descontado: la mercadería salía, la plata no entraba a la caja y
-- tampoco quedaba como deuda de nadie. No aparecía en ningún lado más que en el
-- listado de ventas.
--
-- La pantalla ofrecía ese camino a propósito ("la venta puede quedar
-- pendiente"), así que esto es un cambio de regla, no un bug: lo que el cliente
-- se lleva sin pagar ahora va por CUENTA CORRIENTE, donde la deuda queda a
-- nombre de alguien. La pantalla de ventas se ajusta en el mismo commit.
--
-- Base: la definición VIVA (20260729210000, la del prelock), verificada con
-- diff contra pg_proc. El único cambio es el bloque de abajo.
-- ============================================================
...
BEGIN
  ...
  IF NOT v_es_cta_cte THEN
    ...
    -- CONTADO QUIERE DECIR COBRADO.
    -- Antes, una venta al contado sin pagos quedaba PENDIENTE con
    -- total_pagado = 0: la mercadería salía, la plata no entraba a la caja y
    -- tampoco generaba deuda en la cuenta corriente. No estaba en ningún lado.
    -- Lo que el cliente se lleva sin pagar va por cuenta corriente, que para eso
    -- está: ahí queda la deuda a nombre de alguien.
    --
    ...
    IF p_tipo_comprobante NOT IN ('NOTA_CREDITO', 'NOTA_DEBITO')
       ...
       AND v_pagos_suma < ABS(v_total) - 0.01 THEN
      RAISE EXCEPTION 'Una venta al contado se cobra entera: faltan $%. Si se lo lleva sin pagar, hacela por cuenta corriente.',
        to_char(ABS(v_total) - v_pagos_suma, 'FM999999990.00');
    END IF;
  END IF;
