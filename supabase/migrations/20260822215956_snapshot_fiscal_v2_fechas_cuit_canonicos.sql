-- Paridad estricta con SnapshotFiscalV2: CUIT canónico, no nulo lógico y módulo 11.
-- Es un detalle interno del validador/RPC y no se expone a roles API.
CREATE OR REPLACE FUNCTION public.cuit_fiscal_snapshot_valido(p_cuit text)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
STRICT
...
AS $$
  SELECT CASE
    WHEN p_cuit !~ '^[0-9]{11}$' OR p_cuit='00000000000' THEN false
    ELSE (
      CASE 11-(
        (
          pg_catalog.substr(p_cuit,1,1)::integer*5 +
          pg_catalog.substr(p_cuit,2,1)::integer*4 +
          pg_catalog.substr(p_cuit,3,1)::integer*3 +
          pg_catalog.substr(p_cuit,4,1)::integer*2 +
          pg_catalog.substr(p_cuit,5,1)::integer*7 +
          pg_catalog.substr(p_cuit,6,1)::integer*6 +
          pg_catalog.substr(p_cuit,7,1)::integer*5 +
          pg_catalog.substr(p_cuit,8,1)::integer*4 +
          pg_catalog.substr(p_cuit,9,1)::integer*3 +
          pg_catalog.substr(p_cuit,10,1)::integer*2
        ) % 11
      )
        WHEN 11 THEN 0
        WHEN 10 THEN 9
      ...
      END
    ) = pg_catalog.substr(p_cuit,11,1)::integer
  END;
$$;

REVOKE ALL ON FUNCTION public.cuit_fiscal_snapshot_valido(text)
  FROM PUBLIC,anon,authenticated,service_role;
GRANT EXECUTE ON FUNCTION public.cuit_fiscal_snapshot_valido(text) TO service_role;

-- Copia mecánica de 20260822203901. Sólo suma validación explícita de año
-- 0001..9999 y aplica el helper de CUIT a emisor/receptor/asociación/RESERVAR.
...
BEGIN
  ...
  IF v_desconocidas IS NOT NULL
     ...
     OR v_venta->>'fechaComercial' !~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}(\.[0-9]{3})?Z$'
     OR pg_catalog.substr(v_venta->>'fechaComercial',1,4) NOT BETWEEN '0001' AND '9999' THEN
  ...
  IF v_desconocidas IS NOT NULL
     ...
     OR NOT public.cuit_fiscal_snapshot_valido(v_emisor->>'cuit')
     ...
     OR pg_catalog.substr(v_emisor->>'inicioActividades',1,4) NOT BETWEEN '0001' AND '9999'
  ...
  IF v_desconocidas IS NOT NULL
     ...
     OR NOT public.cuit_fiscal_snapshot_valido(v_identidad->>'emisorCuit')
  ...
  IF pg_catalog.jsonb_typeof(p_snapshot->'letra') IS DISTINCT FROM 'string'
     ...
     OR pg_catalog.substr(p_snapshot->>'fechaComprobante',1,4) NOT BETWEEN '0001' AND '9999'
  ...
  IF pg_catalog.jsonb_typeof(v_receptor->'verificadoArcaAt')='string' THEN
    IF v_receptor->>'verificadoArcaAt' !~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}(\.[0-9]{3})?Z$'
       OR pg_catalog.substr(v_receptor->>'verificadoArcaAt',1,4) NOT BETWEEN '0001' AND '9999' THEN
  ...
  ELSE
    ...
    IF pg_catalog.jsonb_typeof(v_receptor->'numeroDocumento')<>'string'
       ...
       OR v_documento !~ '^[0-9]+$'
       OR (v_receptor->>'tipoDocumento'='CUIT' AND (v_receptor->>'docTipoArca'<>'80' OR NOT public.cuit_fiscal_snapshot_valido(v_documento)))
  ...
  FOR v_asoc IN SELECT value FROM pg_catalog.jsonb_array_elements(p_snapshot->'cbtesAsoc') LOOP
    ...
    IF v_desconocidas IS NOT NULL OR NOT (v_asoc ?& ARRAY['tipo','puntoVenta','numero','cuit','fecha'])
       ...
       OR NOT public.cuit_fiscal_snapshot_valido(v_asoc->>'cuit')
       OR pg_catalog.jsonb_typeof(v_asoc->'fecha') IS DISTINCT FROM 'string'
       OR v_asoc->>'fecha' !~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}$'
       OR pg_catalog.substr(v_asoc->>'fecha',1,4) NOT BETWEEN '0001' AND '9999' THEN
