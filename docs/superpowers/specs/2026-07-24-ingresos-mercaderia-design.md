...
- **Producto nuevo inline**: `productos` tiene escritura solo-admin (`20260629211713_...sql:132`), así que el alta va por `crear_producto_desde_ingreso`, SECURITY DEFINER, que un no-admin puede ejecutar. El **precio de venta (s/IVA) es opcional** en el alta: si se carga (>0), el producto **nace activo y vendible** en la misma pantalla; si se deja vacío, **nace inactivo** (entra al stock pero no se ve ni se vende) hasta que le carguen el precio ahí mismo o después en `/productos`. Imposible venderlo a $0 por descuido: sin precio no se activa. `/productos` gana un filtro "sin precio" para levantar esos pendientes.
