-- ============================================================
-- R2 — "Factura interna" deja de ser cuenta corriente + condición del emisor.
--
-- R2.a  El comprobante FAC_INTERNA_CTA_CTE se mostraba como "Fac. interna Cta
--       Cte" y se forzaba a cuenta corriente. El negocio aclaró: "Fac interna no
--       es de cta corriente". Pasa a ser un documento interno de CONTADO (impacta
--       caja). NO se renombra el valor del enum (usado en funciones/índices/
--       numeración FICC); sólo cambia el comportamiento y el label de la UI.
--       Sigue siendo NO fiscal (TIPOS_INTERNOS lo excluye de AFIP).
--
-- R2.b  Se expone la condición de IVA del emisor a la UI de ventas para poder
--       ofrecer Factura C sólo cuando el emisor es Monotributo. La tabla
--       fiscal_config no tiene RLS para authenticated (guarda el certificado);
--       exponemos SÓLO la condición (dato no secreto) con una función
--       SECURITY DEFINER acotada.
--
-- crear_venta: CREATE OR REPLACE partiendo de la versión de R3 (20260721110000).
-- ============================================================
...
-- R2.b — condición de IVA del emisor (no secreto). Habilita ofrecer Factura C en
-- la UI cuando el emisor es Monotributo, sin exponer el resto de fiscal_config.
CREATE OR REPLACE FUNCTION public.condicion_iva_emisor()
RETURNS text
LANGUAGE sql
STABLE
SECURITY DEFINER
SET search_path TO 'public'
AS $$
  SELECT condicion_iva FROM public.fiscal_config WHERE id = true;
$$;

REVOKE ALL ON FUNCTION public.condicion_iva_emisor() FROM public;
GRANT EXECUTE ON FUNCTION public.condicion_iva_emisor() TO authenticated;
GRANT EXECUTE ON FUNCTION public.condicion_iva_emisor() TO service_role;
